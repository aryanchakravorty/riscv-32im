...
// I-Cache miss: Stall the front-end (IF and ID). Insert NOP into EX.
...
fetch u_fetch (
    ...
    .branch_taken(fetch_redirect_taken),.branch_target(fetch_redirect_target),
    .btb_update_en(btb_update_en),.btb_update_pc(btb_update_pc),
    .btb_actual_taken(btb_actual_taken),.btb_actual_target(btb_actual_target),
    ...
    .predicted_taken_out(if_predicted_taken),.btb_target_out(if_btb_target),.btb_pc_out(if_btb_pc)
);
...
decode u_decode (
    ...
    .predicted_taken_in(if_predicted_taken),.btb_target_in(if_btb_target),.btb_pc_in(if_btb_pc),
    ...
    .rd_addr_out(id_rd_addr),.opcode_out(id_opcode),.funct3_out(id_funct3),.funct7_bit5_out(id_funct7_bit5),
    ...
    .is_fpu_ext_out(id_is_fpu_ext),.fpu_op_out(id_fpu_op),
    .predicted_taken_out(id_predicted_taken),.btb_target_out(id_btb_target),.btb_pc_out(id_btb_pc),
    .valid_out(id_valid)
);
...
execute u_execute (
    ...
    .opcode_in(id_opcode),.funct3_in(id_funct3),.funct7_bit5_in(id_funct7_bit5),
    ...
    .predicted_taken_in(id_predicted_taken),.btb_target_in(id_btb_target),.btb_pc_in(id_btb_pc),
    ...
    .rd_addr_out(ex_rd_addr),.funct3_out(ex_funct3),.is_fpu_ext_out(ex_is_fpu_ext),.fpu_op_out(ex_fpu_op),
    .mem_write_out(ex_mem_write),.mem_read_out(ex_mem_read),
    .mem_to_reg_out(ex_mem_to_reg),.reg_write_out(ex_reg_write),
    .predicted_taken_out(ex_predicted_taken),.btb_target_out(ex_btb_target),.btb_pc_out(ex_btb_pc),
    .valid_out(ex_valid),
...
memory u_memory (
    .clk(clk),.reset(reset),.stall(stall_mem),.flush(flush_mem),
    .pc_plus4_in(ex_pc_plus4),.alu_result_in(ex_alu_result),.rs2_data_in(ex_rs2_data),
    .rd_addr_in(ex_rd_addr),.funct3_in(ex_funct3),.is_fpu_ext_in(ex_is_fpu_ext),.fpu_op_in(ex_fpu_op),
    ...
    .rd_addr_out(mem_rd_addr),.funct3_out(mem_funct3),.is_fpu_ext_out(mem_is_fpu_ext),.fpu_op_out(mem_fpu_op),
    .mem_to_reg_out(mem_mem_to_reg),.reg_write_out(mem_reg_write),.valid_out(mem_valid),
    .forward_data(mem_forward_data),
    .dcache_read_data(dcache_read_data),.dcache_stall(dcache_stall),
    .dcache_addr(dcache_addr),.dcache_wdata(dcache_wdata),.dcache_wstrobe(dcache_wstrobe),
    .dcache_read_en(dcache_read_en),.dcache_write_en(dcache_write_en)
);
...
writeback u_writeback (
    ...
    .rd_addr_in(mem_rd_addr),.funct3_in(mem_funct3),.is_fpu_ext_in(mem_is_fpu_ext),.fpu_op_in(mem_fpu_op),
